...
const SearchResultsScreen = () => {
  ...
  const renderMasonry = () => (
    <View style={styles.masonryContainer}>
      <View style={styles.column}>{leftCol.map((item) => renderCard(item))}</View>
      <View style={styles.column}>{rightCol.map((item) => renderCard(item))}</View>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        />
        <TouchableOpacity style={styles.goButton} onPress={handleSearch}>
          <Ionicons name="arrow-forward" size={18} color={colors.cardsbackground} />
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  input: { flex: 1, color: colors.text, paddingVertical: 10, fontSize: 15 },
